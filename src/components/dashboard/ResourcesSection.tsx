        ...
        {resources.map((resource, index) => (
          <FadeIn key={resource.title} delay={100 + index * 50}>
            <button
              ...
              className="w-full bg-green p-4 md:p-6 rounded-xl shadow-md border border-green/80 text-left transition-all duration-300 hover:shadow-lg button-hover text-cream"
            >
              <div className="w-12 h-12 rounded-full bg-cream/30 text-cream flex items-center justify-center mb-4">
                {resource.icon}
              </div>
              <h3 className="text-lg font-semibold mb-2">{resource.title}</h3>
              <p className="text-sm text-cream/90">{resource.description}</p>
            </button>
          </FadeIn>
        ))}
      </div>
